 ...
 *
 * <p>
 * Bootstraps logging, telemetry, localization, and command wiring before
 * delegating execution to
 * Picocli.
 */
@Command(name = "ful", mixinStandardHelpOptions = true, version = "1.0", resourceBundle = "messages", description = "${cli.description}", footer = "${cli.footer}")
public class Main implements Callable<Integer>, CliContext {
  ...
  @Option(names = { "-c",
      "--config" }, description = "${option.config.description}", defaultValue = ConfigPathResolver.DEFAULT_CONFIG_FILE_NAME)
  private Path configFile;

  @Option(names = { "-l", "--lang" }, description = "${option.lang.description}", paramLabel = "<lang>")
  ...
  private Main(final TelemetryPort telemetry) {
    this(
        new DefaultServiceFactory(telemetry.getTracer()),
   ...
   *
   * <p>
   * Sets up the execution environment, including logging and telemetry, before
   * parsing
   * and executing the given command-line arguments. Returns an appropriate exit
   * code.
   *
   ...
   *
   * <p>
   * Applies locale pre-processing, registers subcommands through
   * {@link ServiceLoader}, and
