...
public class ResultVerifier {
  ...
  private void printHeader(
    ...
    Logger.stdout(
        com.craftsmanbro.fulcraft.i18n.MessageSource.getMessage(
            "analysis.common.log.message",
            "Classes\t\t" + primaryClasses + "\t\t" + secondaryClasses));
    Logger.stdout(
        com.craftsmanbro.fulcraft.i18n.MessageSource.getMessage(
            "analysis.common.log.message",
            "Methods\t\t" + primaryMethods + "\t\t" + secondaryMethods));
  }
  ...
  private void compareAndPrintDetails(
    ...
    for (final String fqn : allClasses) {
      ...
      collectClassMetadataDiffs(fqn, primaryClassInfo, secondaryClassInfo, classMetadataMismatches);
    }
  ...
  private Map<String, ClassInfo> toClassMap(final AnalysisResult result) {
    ...
    return result.getClasses().stream()
        .collect(Collectors.toMap(ClassInfo::getFqn, Function.identity(), (a, b) -> a));
  }
