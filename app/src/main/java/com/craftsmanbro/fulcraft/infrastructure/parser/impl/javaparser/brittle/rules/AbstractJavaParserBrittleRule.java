 ...
 *
 * <p>
 * This class uses JavaParser to analyze test code at the AST level, avoiding
 * false positives
 * from comments, string literals, or import statements that regex-based
 * detection would catch.
 *
...
public abstract class AbstractJavaParserBrittleRule implements BrittleRule {
   ...
   *
   * <p>
   * This method exists for backward compatibility with the interface, but
   * AST-based rules
   * require a parsed {@link CompilationUnit}. The adapter should call
   * {@link #checkAst} instead.
   */
  @Override
  ...
  private static boolean isPotentiallyApplicable(
    ...
    final boolean hasVarArgs = parameterCount > 0 && methodDeclaration.getParameter(parameterCount - 1).isVarArgs();
    if (hasVarArgs) {
      ...
      final Type elementType = extractVarArgElementType(methodDeclaration.getParameter(parameterCount - 1));
    ...
    for (int index = 0; index < parameterCount; index++) {
      if (!isArgumentPotentiallyCompatible(methodDeclaration.getParameter(index), arguments.get(index))) {
        return false;
      }
    }
    return true;
  ...
  private enum InferredExpressionType {
    ...
    private boolean isCompatibleWithPrimitive(final PrimitiveType primitiveType) {
      return switch (this) {
        ...
        case INTEGER ->
          isOneOf(
              primitiveType.asString(), "byte", "short", "char", "int", "long", "float",
              "double");
   ...
   *
   * <p>
   * Subclasses can extend this to add specific detection logic.
   */
