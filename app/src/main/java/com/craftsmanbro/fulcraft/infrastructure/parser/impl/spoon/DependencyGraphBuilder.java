...
/**
 * Builds the dependency graph (call graph) for Spoon analysis. Extracted from
 * SpoonAnalyzer.
 */
public class DependencyGraphBuilder {
  ...
  /**
   * Collects method calls from a given executable and updates the analysis
   * context.
   *
   * @param executable       The Spoon executable (method/constructor) to
   *                         traverse.
  ...
  public void collectCalledMethods(
    ...
    for (final var ref : executable.getElements(new TypeFilter<>(CtExecutableReferenceExpression.class))) {
  ...
  private ResolvedCall resolveMethodCall(final CtInvocation<?> invocation) {
    ...
    final String declaringType = resolveDeclaringType(
    ...
    final String signature = executableReference != null && StringUtils.isNotBlank(executableReference.getSignature())
  ...
  private ResolvedCall resolveMethodReference(
    ...
    final String signature = executableReference != null && StringUtils.isNotBlank(executableReference.getSignature())
        ? executableReference.getSignature()
        : executableReferenceExpression.toString();
    return new ResolvedCall(
  ...
  private String extractLiteralArgument(final CtExpression<?> argument) {
    ...
    if (argument instanceof CtLiteral<?>) {
      return normalizeLiteralSnippet(argument.toString());
    }
    if (argument instanceof CtUnaryOperator<?> unaryOperator && isSignedLiteral(unaryOperator)) {
      return normalizeLiteralSnippet(unaryOperator.toString());
    }
    return null;
  }
  ...
  private void addEdge(
    ...
    context.recordCallArgumentLiterals(
        callerKey, target.signature(), target.argumentLiterals());
  }
