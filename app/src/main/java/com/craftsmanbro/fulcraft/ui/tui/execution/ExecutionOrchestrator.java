 ...
 *
 * <p>
 * This class bridges the TUI and the existing pipeline infrastructure,
 * providing:
 *
 * <ul>
...
public class ExecutionOrchestrator {
  ...
  private static final List<String> ANALYSIS_REPORT_DOCUMENT_STEPS = List.of(PipelineNodeIds.ANALYZE,
      PipelineNodeIds.REPORT, PipelineNodeIds.DOCUMENT);
  ...
  public static ExecutionOrchestrator create(
    ...
    final Config config = configFile != null ? configLoader.load(configFile) : Config.createDefault();
   ...
   *
   * <p>
   * The execution runs in a background thread, updating the session with progress
   * and logs.
   */
  ...
  private int runAnalyzeReportDocument(final RunContext context)
    ...
    final RunAnalysisReportExecutor analysisExecutor = new RunAnalysisReportExecutor(services, DEFAULT_ENGINE_TYPE);
  ...
  private RunContext createContext() {
    ...
    final RunContext context = new RunContext(projectRoot.toAbsolutePath(), config, runId)
        .withDryRun(false)
        .withFailFast(false)
